...
export const getPlanByUser = asyncHandler(async (req, res) => {
    const user = req.user;
    if (!user || !user.id) {
...
export const getPlanDestinationById = asyncHandler(async (req, res) => {
    const user = req.user;
    if (!user || !user.id) {
        throw new ApiError(401, "Unauthorized access");
    }
...
export const updatePlanById = asyncHandler(async (req, res) => {
    const user = req.user;
    if (!user || !user.id) {
