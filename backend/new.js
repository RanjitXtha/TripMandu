...
async function fetchWikipediaData(place) {
  ...
  try {
    ...
    if (!bestMatch) {
      return { name: place.name, category: place.category, error: "No Wikipedia match found." };
    }

    const summaryUrl = `https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(bestMatch)}`;
    const summaryRes = await fetch(summaryUrl);
    if (!summaryRes.ok) {
      return { name: place.name, category: place.category, error: "Summary not found." };
    }

    const data = await summaryRes.json();

    // Infer tags from description in addition to category
    const desc = (data.extract || "").toLowerCase();
    const tags = [place.category];
    if (desc.includes("temple") || desc.includes("stupa") || desc.includes("buddha")) tags.push("religious");
    if (desc.includes("durbar") || desc.includes("palace") || desc.includes("historic") || desc.includes("museum")) tags.push("historic");
    if (desc.includes("market") || desc.includes("bazaar")) tags.push("market");
    if (desc.includes("garden") || desc.includes("lake") || desc.includes("pond")) tags.push("natural");
    if (desc.includes("monastery") || desc.includes("gompa")) tags.push("monastery");
    ...
    return {
      ...
      tags: Array.from(new Set(tags)) // unique tags only
    };
  } catch (err) {
    return { name: place.name, category: place.category, error: err.message };
  }
}
